...
function collectResults(
    { name, duration, runDuration, benchmarks },
    collector,
) {
    const cNode =
        collector[name] ||
        (collector[name] = { duration: [], runDuration: [] });
    if (duration > 0) {
...
function createStructure({ benchmarks, name, runDuration }, collector) {
    ...
    return {
        name,
        benchmarks: benchmarks.map(childNode =>
            createStructure(childNode, collector),
        ),
    };
}
...
export async function analyzeBenchmarks(benchmarkResults) {
    return Promise.all(
        benchmarkResults.map(async benchmarkResult => {
            ...
            const collector = results.reduce(
                (c, result) => collectResults(result, c),
                {},
            );

            const benchmarkStats = Object.keys(collector).reduce(
                (stats, bName) => {
                    const benchmarkMetrics = collector[bName];
                    stats[bName] = Object.keys(benchmarkMetrics).reduce(
                        (mc, metric) => {
                            const list = benchmarkMetrics[metric];
                            if (Array.isArray(list)) {
                                if (list.length) {
                                    mc[metric] = computeSampleStats(
            ...
            const benchmarkStructure = createStructure(
                structure,
                benchmarkStats,
            );
