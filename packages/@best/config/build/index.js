...
function buildTestPathPattern(argsCLI) {
    ...
    const testPathPattern = patterns.map(regex_util_1.replacePathSepForRegex).join('|');
...
function normalize(options, argsCLI) {
    ...
    const newOptions = Object.assign({}, defaults_1.default);
...
async function readConfig(argsCLI, packageRoot, parentConfigPath) {
    ...
    if (!parentConfigPath) {
        try {
            await git_1.addGitInformation(options);
        }
        catch (e) {
...
async function getConfigs(projectsFromCLIArgs, argv) {
    let globalConfig;
    let configs = [];
