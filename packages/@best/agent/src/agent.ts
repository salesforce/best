...
export class Agent extends EventEmitter {
    ...
    onClientConnect(socketClient: ServerSocket) {
        ...
        console.log(
            `[AGENT] Connected clients: ${this.connectedClients.size} | state: ${this.state} | activePending: ${
                this.activeClient && this.activeClient.getPendingBenchmarks()
            }`,
        );
    ...
    async runBenchmark(remoteClient: RemoteClient) {
        if (this.idleState) {
            ...
            try {
                ...
                console.log(
                    `[AGENT] Running benchmark ${
                        benchmarkBuild.benchmarkSignature
                    } from RemoteClient ${remoteClient.getId()}`,
                );
    ...
    runQueuedBenchmarks() {
        if (this.idleState) {
            ...
            if (this.activeClient && this.activeClient.getPendingBenchmarks()) {
                console.log(
                    `[AGENT] Active Client "${this.activeClient.getId()}" has still ${this.activeClient.getPendingBenchmarks()} pending`,
                );
                ...
                const remoteClient = Array.from(this.connectedClients).find(
                    (client) => client.getPendingBenchmarks() > 0,
                );

                if (remoteClient) {
                    console.log(
                        `[AGENT] Client "${remoteClient.getId()}" has ${remoteClient.getPendingBenchmarks()} to run`,
                    );
                    this.runBenchmark(remoteClient);
                } else {
    ...
    setupNewClient(socketClient: ServerSocket, clientConfig: RemoteClientConfig): RemoteClient {
        ...
        console.log(
            `[AGENT] New client ${remoteClient.getId()} connected. Jobs requested ${
                clientConfig.jobs
        ...
        remoteClient.on(BEST_RPC.REMOTE_CLIENT_EMPTY_QUEUE, () => {
            setTimeout(() => {
                if (this.connectedClients.has(remoteClient)) {
                    console.log(
                        `[AGENT] Force client disconnect (${remoteClient.getId()}): With no more jobs to run an agent must disconnect`,
                    );
