...
import {
    ...
    BenchmarkStats,
    AllBenchmarksMetricsMap,
    BenchmarkMetricsAggregate,
    AllBenchmarkMetricStatsMap,
    StatsNode,
    BenchmarkMetricStatsMap,
    StatsNodeGroup,
    MetricsStatsMap,
    FrozenProjectConfig,
} from '@best/types';
...
function collectResults(
    resultNode: BenchmarkResultNode,
    collector: AllBenchmarksMetricsMap,
    projectConfig: FrozenProjectConfig,
) {
    const { name } = resultNode;
    let collectorNode = collector[name];
    if (!collectorNode) {
        const emptyMetrics = projectConfig.metrics.reduce(
            (acc, key) => ({
                ...acc,
...
export async function analyzeBenchmarks(benchmarkResults: BenchmarkResultsSnapshot[]) {
    return Promise.all(
        ...
        benchmarkResults.map(async (benchmarkResult: BenchmarkResultsSnapshot) => {
            ...
            const collector: AllBenchmarksMetricsMap = results.reduce(
                (reducer: AllBenchmarksMetricsMap, node: BenchmarkResultNode) =>
                    collectResults(node, reducer, projectConfig),
                {},
            );

            // For each metric
            const benchmarkStats: AllBenchmarkMetricStatsMap = Object.keys(collector).reduce(
                (stats: AllBenchmarkMetricStatsMap, benchmarkName: string) => {
                    const benchmarkMetrics = collector[benchmarkName];

                    stats[benchmarkName] = Object.keys(benchmarkMetrics).reduce(
                        (metricStats: BenchmarkMetricStatsMap, metric: string) => {
                            const metricResults = benchmarkMetrics[metric as BenchmarkMetricNames];
                            if (Array.isArray(metricResults) && metricResults.length > 0) {
                                metricStats[metric as BenchmarkMetricNames] = computeSampleStats(
                                    metricResults,
                                    projectConfig.samplesQuantileThreshold,
                                );
                            }
                            return metricStats;
                        },
