...
function compareBenchmarks(baseBenchs, targetBenchs, comparison = []) {
    if (baseBenchs && baseBenchs.length && targetBenchs && targetBenchs.length) {
        baseBenchs.forEach((baseBenchmark) => {
            ...
            if (!targetBenchmark) {
                console.log(`Skipping benchmark test ${baseBenchmark.name} since we couldn't find it in target.` +
            ...
            if (baseBenchmark.benchmarks) {
                ...
                const durationSampleComparison = analyzer_1.compareSamples(baseDurationMetrics.samples, targetDurationMetrics.samples);
                comparison.push({
                    ...
                    metrics: {
                        duration: {
                            ...
                            samplesComparison: durationSampleComparison,
                        },
                    },
                });
            }
...
async function compareBenchmarkStats(baseCommit, targetCommit, projectNames, storageProvider) {
    const stats = await Promise.all(projectNames.reduce((reducer, projectName) => [
        ...reducer,
