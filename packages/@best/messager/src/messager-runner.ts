...
const printBenchmarkEntry = (benchmarkEntry: string) => {
...
const printDisplayPath = (relativeDir: string) => {
    if (relativeDir) {
...
const printError = ({ benchmarkEntry }: any) => {
    return [
...
const generateProgressState = (progress: any, { iterations, maxDuration }: any) => {
    const { executedIterations, executedTime }: any = progress;
...
const renderTime = (runTime: any, estimatedTime: any, width: any) => {
...
export default class RunnerMessager {
    ...
    _out: any;
    _state: { benchmarks: any; buffer: string; progress?: any };
    _currentState: any;
    _queued: any;
    constructor(benchmarksBundle: any, globalConfig: any, outputStream: any) {
        ...
        const benchmarksState = benchmarksBundle.reduce((map: any, { benchmarkName, benchmarkEntry, projectConfig }: any) => {
    ...
    onBenchmarkStart(benchmarkName: string, projectName: string, overrideOpts: any) {
