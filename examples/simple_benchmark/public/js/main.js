...
function init(modules, domApi) {
    ...
    function createElm(vnode, insertedVnodeQueue) {
        ...
        if (sel === '!') {
            ...
            for (i$$1 = 0; i$$1 < cbs.create.length; ++i$$1) cbs.create[i$$1](emptyNode, vnode);
            if (array(children)) {
    ...
    function invokeDestroyHook(vnode) {
        ...
        if (data !== undefined) {
            ...
            for (i$$1 = 0; i$$1 < cbs.destroy.length; ++i$$1) cbs.destroy[i$$1](vnode);
    ...
    function patchVnode(oldVnode, vnode, insertedVnodeQueue) {
        ...
        if (vnode.data !== undefined) {
            for (i$$1 = 0; i$$1 < cbs.update.length; ++i$$1) cbs.update[i$$1](oldVnode, vnode);
            i$$1 = vnode.data.hook;
    ...
    return function patch(oldVnode, vnode) {
        ...
        for (i$$1 = 0; i$$1 < cbs.pre.length; ++i$$1) cbs.pre[i$$1]();
        if (!isVnode(oldVnode)) {
        ...
        for (i$$1 = 0; i$$1 < cbs.post.length; ++i$$1) cbs.post[i$$1]();
        return vnode;
    };
}
...
if (style) {
   const tagName = 'simple-benchmark';
   const token = 'simple-benchmark_simple-benchmark';

   tmpl.token = token;
   tmpl.style = style(tagName, token);
}

class SimpleBench extends ComponentElement {
  render() {
    return tmpl;
  }
